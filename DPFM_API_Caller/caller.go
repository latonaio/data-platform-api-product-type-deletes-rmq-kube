...
import (
	...
	"data-platform-api-product-type-deletes-rmq-kube/config"
...
func (c *DPFMAPICaller) deleteSqlProcess(
	...
	var productTypeData *dpfm_api_output_formatter.ProductType
	for _, a := range accepter {
		switch a {
		case "ProductType":
			h := c.productTypeDelete(input, output, log)
			productTypeData = h
			if h == nil {
				continue
			}
...
func (c *DPFMAPICaller) productTypeDelete(
	input *dpfm_api_input_reader.SDC,
	output *dpfm_api_output_formatter.SDC,
	log *logger.Logger,
) *dpfm_api_output_formatter.ProductType {
	sessionID := input.RuntimeSessionID
	productType := c.ProductTypeRead(input, log)
	productType.IsMarkedForDeletion = input.ProductType.IsMarkedForDeletion
	res, err := c.rmq.SessionKeepRequest(nil, c.conf.RMQ.QueueToSQL()[0], map[string]interface{}{"message": productType, "function": "ProductTypeProductType", "runtime_session_id": sessionID})
	if err != nil {
	...
	if !checkResult(res) {
		...
		output.SQLUpdateError = "ProductType Data cannot delete"
		return nil
	}
